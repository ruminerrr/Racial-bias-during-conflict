...
erp_mean_Stimulus(:,:,:,:) = nan(SubjectNum,size(StimCases,2), EEG.nbchan, (Segementation_time_end-Segementation_time_start)*EEG.srate, 'single');		    %4D array: VP,CASES,ELECTRODES,TIMES
...
Casecheck (:,:) = zeros(SubjectNum,size(StimCases,2), 'single');
...
for P = 1:SubjectNum
    ...
    for CASES = 1:size(StimCases,2)
        ...
        try % see whether the relevant segements are there... else do the next iteration
            ...
            EEG = pop_select(EEG,'time',[-1.4 7]);
            EEG = pop_epoch(EEG, StimCases{CASES},[-1 2]);
            ...
            erp_mean_Stimulus(P,CASES,1:size(EEG.data,1),1:size(EEG.data,2))  = single(nanmean(EEG.data,3));	            %4D array: VP,CASES,ELECTRODES,TIMES
...
size(erp_mean_Stimulus)
StimERP = erp_mean_Stimulus(:,:,:,1:750);
save('StimERP', '-v7.3')
