...
erp_mean_ChoiceOnset(:,:,:,:) = nan(SubjectNum,size(OnsetCases,2), EEG.nbchan, (Segementation_time_end-Segementation_time_start)*EEG.srate, 'single');		    %4D array: VP,CASES,ELECTRODES,TIMES
...
Casecheck (:,:) = zeros(SubjectNum,size(OnsetCases,2), 'single');
...
for P = 1:SubjectNum
	...
	for CASES = 1:size(OnsetCases,2) 
        ...
        try % see whether the relevant segements are there... else do the next iteration
            ...
            EEG = pop_select(EEG,'time',[-1.4 9]);
            EEG = pop_epoch(EEG, OnsetCases{CASES},[-1 2]);
...
OnsetERP = erp_mean_ChoiceOnset(:,:,:,1:750);
save('OnsetERP', '-v7.3')